...
class DownloadManager:
    ...
    def run(self):
        ...
        with ThreadPool(cpu_count())as threads:
            jobs = threads.starmap(download_spotify, zip(self.queue, repeat(self.quality), repeat(self.download_format),
            ...
            message = (f'Download Finished! \nCompleted {len(self.queue) - len(failed_jobs)}/{len(self.queue)} tracks '
